...
//
NS_LOG_COMPONENT_DEFINE ("edgecloud");
...
main (int argc, char *argv[])
{
  ...
  for (uint32_t i = 0; i < nWifis; ++i)
  {
	  ...
	  for (uint32_t m= 0; m < nStas; ++m)
	  {
  	  ...
  	  wifiPhy.EnablePcap ("edgecloud", appSink->GetId (), 0);
	  }
  }
  ...
  Ptr<OutputStreamWrapper> stream = ascii.CreateFileStream ("edgecloud.tr");
  wifiPhy.EnableAsciiAll (stream);
  //p2p.EnableAsciiAll (stream);
  internet.EnableAsciiIpv4All (stream);

  // p2p captures in non-promiscuous mode
  p2p.EnablePcapAll ("edgecloud", false);
  //pcap captures on the backboneNodes wifi devices
  wifiPhy.EnablePcap ("edgecloud", backboneNodesDevices, false);


  AnimationInterface anim ("edgecloud.xml");
