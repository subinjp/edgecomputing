...
//
NS_LOG_COMPONENT_DEFINE ("edgecomp_Stream");
...
main (int argc, char *argv[])
{
  ...
  for (uint32_t i = 0; i < nWifis; ++i)
  {
  	  ...
  	  //get the index of the source node
  	  uint32_t firstNodeIndex = nWifis +i;
  	  Ptr<Node> appSource = NodeList::GetNode (firstNodeIndex);
  	  Ipv4Address sourceAddr = appSource->GetObject<Ipv4> ()->GetAddress (1, 0).GetLocal ();
  	  ...
  	  UdpTraceClientHelper client (sourceAddr,9,"");
  	  //UdpEchoClientHelper echoClient (sourceAddr, 9);
  	  ...
  	  wifiPhy.EnablePcap ("edgecomp_Stream", appSink->GetId (), 0);
  	  }
  ...
  Ptr<OutputStreamWrapper> stream = ascii.CreateFileStream ("edgecomp_Stream.tr");
  wifiPhy.EnableAsciiAll (stream);
  //p2p.EnableAsciiAll (stream);
  internet.EnableAsciiIpv4All (stream);

  // p2p captures in non-promiscuous mode
  p2p.EnablePcapAll ("edgecomp_Stream", false);
  //pcap captures on the backboneNodes wifi devices
  //wifiPhy.EnablePcap ("edgecomp_Stream", backboneNodesDevices, false);


  AnimationInterface anim ("edgecomp_Stream.xml");
