...
//
//		backboneNodes: starting IP Address:192.168.0.0 Subnet Mask:255.255.255.0
//		P2P Nodes:     starting IP Address:172.16.0.0  Subnet Mask:255.255.255.0
//		WirelessNodes: starting IP Address:10.0.0.0    Subnet Mask:255.255.255.0
//          +--------------------------------------------------------+
//          |                                                        |
//          |              CSMA-------------CSMA            |
//          |                                                        |
...
main (int argc, char *argv[])
{
  ...
  for (uint32_t i = 0; i < nWifis; ++i)
  {
  	  ...
  	  //get the index of the source node
  	  uint32_t firstNodeIndex = nWifis+2;
  	  Ptr<Node> appSource = NodeList::GetNode (firstNodeIndex);
  	  Ipv4Address sourceAddr = appSource->GetObject<Ipv4> ()->GetAddress (1, 0).GetLocal ();
  	  ...
  	  UdpTraceClientHelper client (sourceAddr,9,"/home/sujith/ns-allinone-3.25/ns-3.25/scratch/robinhood.dat");
  	  //UdpEchoClientHelper echoClient (sourceAddr, 9);
  	  //echoClient.SetAttribute ("MaxPackets", UintegerValue (1));
  	  //echoClient.SetAttribute ("Interval", TimeValue (Seconds (3.0)));
  	  client.SetAttribute ("MaxPacketSize", UintegerValue (MaxPacketSize));

  	  ApplicationContainer clientApps = client.Install (appSink);
  	  clientApps.Start (Seconds (2.0));
  	  clientApps.Stop (Seconds (20.0));
  	  j+=1;
  	  // pcap trace on the application data sink
  	  //wifiPhy.EnablePcap ("cloud_Stream", appSink->GetId (), 0);
  	  }
  }
  ...
  Ptr<OutputStreamWrapper> stream = ascii.CreateFileStream ("cloud_Stream.tr");
  wifiPhy.EnableAsciiAll (stream);
  //p2p.EnableAsciiAll (stream);
  internet.EnableAsciiIpv4All (stream);

  // p2p captures in non-promiscuous mode
  p2p.EnablePcapAll ("cloud_Stream");
  //pcap captures on the backboneNodes wifi devices
  //wifiPhy.EnablePcap ("cloud_Stream", backboneNodesDevices, false);
  wifiPhy.EnablePcapAll("cloud_Stream");

  AnimationInterface anim ("cloud_Stream.xml");
