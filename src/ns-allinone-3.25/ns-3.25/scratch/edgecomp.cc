...
//
NS_LOG_COMPONENT_DEFINE ("edgecomp");
...
main (int argc, char *argv[])
{
  ...
  for (uint32_t i = 0; i < nWifis; ++i)
  {
  	  ...
  	  wifiPhy.EnablePcap ("edgecomp", appSink->GetId (), 0);
  	  }
  }
  ...
  Ptr<OutputStreamWrapper> stream = ascii.CreateFileStream ("edgecomp.tr");
  wifiPhy.EnableAsciiAll (stream);
  //p2p.EnableAsciiAll (stream);
  internet.EnableAsciiIpv4All (stream);

  // p2p captures in non-promiscuous mode
  p2p.EnablePcapAll ("edgecomp", false);
  //pcap captures on the backboneNodes wifi devices
  wifiPhy.EnablePcap ("edgecomp", backboneNodesDevices, false);


  AnimationInterface anim ("edgecomp.xml");
