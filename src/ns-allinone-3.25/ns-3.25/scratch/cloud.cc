...
main (int argc, char *argv[])
{
  ...
  for (uint32_t i = 0; i < nWifis; ++i)
  {
  	  ...
  	  wifiPhy.EnablePcap ("cloud", appSink->GetId (), 0);
  	  }
  }
  ...
  Ptr<OutputStreamWrapper> stream = ascii.CreateFileStream ("cloud.tr");
  wifiPhy.EnableAsciiAll (stream);
  //p2p.EnableAsciiAll (stream);
  internet.EnableAsciiIpv4All (stream);

  // p2p captures in non-promiscuous mode
  p2p.EnablePcapAll ("cloud", false);
  //pcap captures on the backboneNodes wifi devices
  wifiPhy.EnablePcap ("cloud", backboneNodesDevices, false);


  AnimationInterface anim ("cloud.xml");
